...
if [[ "$OS" = "Ubuntu" ]]; then
    sudo curl --fail https://signup.volumez.com/connector/ubun/ubun_vlzconnector.list -o /etc/apt/sources.list.d/vlzconnector.list || package_not_found $OS
    ...
    sudo curl --fail https://signup.volumez.com/connector/amzn/amzn.repo -o /etc/yum.repos.d/volumez.repo || package_not_found $OS
    sudo mkdir -p /opt/vlzconnector
    echo -n $tenant_token | sudo tee -a /opt/vlzconnector/tenantToken
    sudo yum -y install vlzconnector || failed_vlzconnector_install $OS
elif [[ "$OS" = "SLES" ]]; then
    sudo curl --fail https://signup.volumez.com/connector/sles/sles.repo -o /etc/zypp/repos.d/volumez.repo || package_not_found $OS
    sudo mkdir -p /opt/vlzconnector
    echo -n $tenant_token | sudo tee -a /opt/vlzconnector/tenantToken
    sudo zypper --non-interactive install -y vlzconnector || failed_vlzconnector_install $OS
elif [[ "$OS" = "Red Hat Enterprise Linux" ]]; then
    sudo curl --fail https://signup.volumez.com/connector/rhel/rhel.repo -o /etc/yum.repos.d/volumez.repo || package_not_found $OS
    sudo mkdir -p /opt/vlzconnector
    echo -n $tenant_token | sudo tee -a /opt/vlzconnector/tenantToken
